...
public class grade_dashb extends javax.swing.JFrame {
    ...
    public void displayUsers(){
        ...
        ResultSet rs = db.getData("SELECT e.enrollmentID, e.studentID, g.id AS 'Grade ID', g.course, g.prelim, g.midterm, g.prefinal, g.final " +
                  "FROM tbl_enrollment e LEFT JOIN tbl_grade g ON e.studentID = g.student_id");
    ...
    private void initComponents() {
        ...
        jLabel9.setText("Enrolled Student And Grade List");
    ...
    int rowIndex = enroll_list.getSelectedRow();
        if(rowIndex < 0){
           ...
           try{connectDB db = new connectDB();
           TableModel tbl = enroll_list.getModel();
           ResultSet rs = db.getData("Select * From tbl_grade WHERE student_id = '"+tbl.getValueAt(rowIndex, 1 )+"'");
           if(rs.next()){
             updategrade grade_dashb = new updategrade();
             grade_dashb.student_id.setText(""+rs.getInt("student_id"));
             grade_dashb.prelim.setText(""+rs.getDouble("prelim"));
             grade_dashb.midterm.setText(""+rs.getDouble("midterm"));
             grade_dashb.prefinal.setText(""+rs.getDouble("prefinal"));
             grade_dashb.finalg.setText(""+rs.getDouble("final"));
             grade_dashb.course.setSelectedItem(""+rs.getString("course")); 
             grade_dashb.setVisible(true);
             this.dispose();
         }        
           }catch (SQLException ex){
    ...
    private void jPanel6MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jPanel6MouseClicked
        ...
        if(rowindex < 0){
            ...
            int confirm = JOptionPane.showConfirmDialog(null, "Are you sure you want to delete this student grade?", "Confirm Deletion", JOptionPane.YES_NO_OPTION);
            if (confirm == JOptionPane.YES_OPTION) {
                connectDB dbc = new connectDB();
                TableModel model = enroll_list.getModel();
                String id = model.getValueAt(rowindex, 2).toString();
                dbc.updateData("DELETE FROM tbl_grade WHERE id = '" + id + "'");
